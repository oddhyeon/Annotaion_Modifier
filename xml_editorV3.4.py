...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.edit_cuboid_id = QLineEdit(self)
        self.edit_cuboid_id.setGeometry(130, 230, 100, 30)


        # 슬라이더를 Cuboid ID 아래쪽에 배치
        ...
        self.slider_cuboid_id.valueChanged.connect(self.update_selected_cuboid_id)

        # QLabel 추가 - 선택된 Cuboid ID를 표시
        self.label_cuboid_id_selected = QLabel("Selected ID: 1", self)
        self.label_cuboid_id_selected.setGeometry(20, 249, 360, 30)

        # 슬라이더와 QLabel의 초기값 설정

        self.label_cuboid_id_selected.setText("Selected ID: 1")
        self.slider_cuboid_id.setValue(1)



        # QLineEdit의 textChanged 시그널과 슬라이더의 valueChanged 시그널 연결
        self.edit_cuboid_id.textChanged.connect(self.ignore_edit_cuboid_id_change)
        self.edit_cuboid_id.textChanged.disconnect()
    ...
    def update_selected_cuboid_id(self, value):
        self.label_cuboid_id_selected.setText(f"Selected ID: {value}")

    def ignore_edit_cuboid_id_change(self):
        try:
            # 슬라이더 값으로 QLineEdit 값을 갱신
            cuboid_id = int(self.edit_cuboid_id.text())
            self.slider_cuboid_id.blockSignals(True)
            self.slider_cuboid_id.setValue(cuboid_id)
            self.slider_cuboid_id.blockSignals(False)
        except ValueError:
            pass

    def update_cuboid_id(self, value):
        self.edit_cuboid_id.setText(str(value))

    def update_cuboid_id_from_lineedit(self):
        try:
            cuboid_id = int(self.edit_cuboid_id.text())
            # 슬라이더 범위를 초과하지 않도록 제한
            max_cuboid_id = self.slider_cuboid_id.maximum() if hasattr(self, 'slider_cuboid_id') else 100
            if 1 <= cuboid_id <= max_cuboid_id:
                self.slider_cuboid_id.blockSignals(True)  # 슬라이더 이벤트를 차단하여 반영된 값을 다시 수정하지 않도록 함
                self.slider_cuboid_id.setValue(cuboid_id)
                self.slider_cuboid_id.blockSignals(False)
        except ValueError:
            pass

    def update_cuboid_id_from_slider(self, value):
        self.edit_cuboid_id.setText(str(value))
    ...
    def modify_xml(self):
        if not self.last_selected_files:  # 이전에 선택한 파일들이 없는 경우
            return  # 함수 종료

        start_index = self.slider_start_index.value()
        end_index = self.slider_end_index.value()
        new_cuboid_id = int(self.edit_cuboid_id.text())  # 새로 입력한 Cuboid ID 값
            ...
            try:
                ...
                for obj in root.iter("object"):
                    ...
                    if obj_cuboid_id == self.slider_cuboid_id.value():
                        obj.find("cuboid_id").text = str(new_cuboid_id)

                    # 클래스(Class)와 서브클래스(Subclass) 변경 기능 추가
